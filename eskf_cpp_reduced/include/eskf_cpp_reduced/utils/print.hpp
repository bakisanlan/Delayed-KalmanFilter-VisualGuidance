...
#include "eskf_cpp_reduced/utils/colors.h"

namespace eskf {

/**
 * @brief Printer for ESKF that allows for various levels of printing to be done
 *
 * To set the global verbosity level one can do the following:
 * @code{.cpp}
 * eskf::Printer::setPrintLevel("WARNING");
 * eskf::Printer::setPrintLevel(eskf::Printer::PrintLevel::WARNING);
 * @endcode
 */
class Printer {
public:
 ...
 */
#define PRINT_ALL(...) eskf::Printer::debugPrint(eskf::Printer::PrintLevel::ALL, __FILE__, TOSTRING(__LINE__), __VA_ARGS__);
#define PRINT_DEBUG(...) eskf::Printer::debugPrint(eskf::Printer::PrintLevel::DEBUG, __FILE__, TOSTRING(__LINE__), __VA_ARGS__);
#define PRINT_INFO(...) eskf::Printer::debugPrint(eskf::Printer::PrintLevel::INFO, __FILE__, TOSTRING(__LINE__), __VA_ARGS__);
#define PRINT_WARNING(...) eskf::Printer::debugPrint(eskf::Printer::PrintLevel::WARNING, __FILE__, TOSTRING(__LINE__), __VA_ARGS__);
#define PRINT_ERROR(...) eskf::Printer::debugPrint(eskf::Printer::PrintLevel::ERROR, __FILE__, TOSTRING(__LINE__), __VA_ARGS__);

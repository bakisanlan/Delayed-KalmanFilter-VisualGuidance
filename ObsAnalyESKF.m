...
% Initialize state transition product (Phi_k = Fd_{k-1} * Fd_{k-2} * ... * Fd_0)
Phi_k = eye(17);

% Store Fd matrices for analysis
Fd_history = zeros(17, 17, N_steps);

fprintf('Computing ESKF Observability Gramian over %d steps...\n', N_steps);
...
for k = 1:N_steps
    ...
    if p_zc < 2
        disp('stopped - target too close to camera')
        break
    end    
    
    %% Simulate IMU measurements (corrected for bias)
    omega_meas = omega_true + b_gyr_true;  % Add bias to true omega
    a_meas = a_body_true + b_acc_true;     % Add bias to true acceleration
    ...
    %% Update state transition product: Phi_k = Fd_k * Phi_{k-1}
    Phi_k = Fd_k * Phi_k;
...
fprintf('Singular Values of ESKF Observability Gramian:\n');
for i = 1:17
    fprintf('  σ_%d = %.4e\n', i, singular_values(i));
end
...
% Finding each state's observability by examining basis vector contribution
obs_state = zeros(17,1);
for i = 1:17
    sum_sq = 0;
    for j = 1:17
...
fprintf('\nError State Observability Measures:\n');
state_names = {'δθ_x', 'δθ_y', 'δθ_z', ...
               'δp_rx', 'δp_ry', 'δp_rz', ...
               'δv_rx', 'δv_ry', 'δv_rz', ...
               'δpbar_x', 'δpbar_y', ...
               'δb_gyr_x', 'δb_gyr_y', 'δb_gyr_z', ...
               'δb_acc_x', 'δb_acc_y', 'δb_acc_z'};
for i = 1:17
...
% Row labels (error states)
rowLabels = {'$\delta\theta_x$', '$\delta\theta_y$', '$\delta\theta_z$', ...
             '$\delta p_{rx}$', '$\delta p_{ry}$', '$\delta p_{rz}$', ...
             '$\delta v_{rx}$', '$\delta v_{ry}$', '$\delta v_{rz}$', ...
             '$\delta\bar{p}_x$', '$\delta\bar{p}_y$', ...
             '$\delta b_{\omega x}$', '$\delta b_{\omega y}$', '$\delta b_{\omega z}$', ...
             '$\delta b_{ax}$', '$\delta b_{ay}$', '$\delta b_{az}$'};
...
title('ESKF Observability Analysis - SVD of Gramian', 'FontSize', 14);
...
title('Singular Values of ESKF Observability Gramian (log scale)', 'FontSize', 14);
grid on;
xticks(1:17);
...
bar(log10(obs_state));
xlabel('Error State Index', 'FontSize', 12);
ylabel('log_{10}(Observability Measure)', 'FontSize', 12);
title('ESKF Error State-wise Observability Measure', 'FontSize', 14);
grid on;
xticks(1:17);
xticklabels(state_names);
xtickangle(45);

%% ---- 4. State Group Observability Comparison ----
% Group states by type and compute average observability
figure('Color', 'w', 'Position', [250, 250, 600, 400]);

group_names = {'Attitude (\delta\theta)', 'Position (\delta p_r)', ...
               'Velocity (\delta v_r)', 'Image (\delta\bar{p})', ...
               'Gyro Bias (\delta b_\omega)', 'Accel Bias (\delta b_a)'};
group_obs = [mean(obs_state(idx_dtheta)); 
             mean(obs_state(idx_dpr)); 
             mean(obs_state(idx_dvr)); 
             mean(obs_state(idx_dpbar)); 
             mean(obs_state(idx_dbgyr)); 
             mean(obs_state(idx_dbacc))];

bar(log10(group_obs));
xlabel('State Group', 'FontSize', 12);
ylabel('log_{10}(Mean Observability)', 'FontSize', 12);
title('ESKF Observability by State Group', 'FontSize', 14);
grid on;
xticks(1:6);
xticklabels(group_names);
xtickangle(30);

fprintf('\n=== ESKF Observability Analysis Complete ===\n');

%% ======================== SUMMARY ========================
fprintf('\n=== SUMMARY ===\n');
fprintf('Error State Dimension: 17\n');
fprintf('Simulation Duration: %.2f s\n', N_steps * dt_imu);
fprintf('Condition Number: %.4e\n', cond_number);
fprintf('\nMost Observable States:\n');
[~, sorted_idx] = sort(obs_state, 'descend');
for i = 1:5
    fprintf('  %d. %s: %.4e\n', i, state_names{sorted_idx(i)}, obs_state(sorted_idx(i)));
end
fprintf('\nLeast Observable States:\n');
for i = 17:-1:13
    fprintf('  %d. %s: %.4e\n', 18-i, state_names{sorted_idx(i)}, obs_state(sorted_idx(i)));
end
